...
absl::StatusOr<HFSMasterDirectoryBlock> HFSMasterDirectoryBlock::ReadFromDisk(
  ...
  s.seekg(kMDBOffset);
  if (s.fail()) {
    return absl::OutOfRangeError("Could not seek to HFS Master Directory Block");
  }
...
absl::StatusOr<uint64_t> HFSMasterDirectoryBlock::Valid() const {
  ...
  if (mod != 0) {
    return absl::FailedPreconditionError(
	 absl::StrFormat("Declared allocation size %d not a multiple of"
			 "block size %d", allocation_block_size_,
			 kHFSBlockSize));
  }
